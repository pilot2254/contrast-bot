...
export class StatsManager {
  ...
  async getDatabaseSize(): Promise<number> {
    const pageCountResult = await this.client.database.get<{
      page_count: number
    }>("PRAGMA page_count")
    const pageSizeResult = await this.client.database.get<{
      page_size: number
    }>("PRAGMA page_size")
  ...
  async getUserCount(): Promise<number> {
    const result = await this.client.database.get<{ count: number }>(
  ...
  async getEconomyStats(): Promise<EconomyStats> {
    ...
    const richestUserResult = await this.client.database.get<{
      user_id: string
      total_balance: number
    }>(
      "SELECT user_id, balance + safe_balance as total_balance FROM users ORDER BY total_balance DESC LIMIT 1"
    )
    const averageBalanceResult = await this.client.database.get<{
      average: number
    ...
    return {
      ...
      richestUser: richestUserResult
        ? {
            userId: richestUserResult.user_id,
            balance: richestUserResult.total_balance,
          }
