...
export class StatsManager {
  ...
  async getTotalCommandsUsed(): Promise<number> {
    const result = await this.client.database.get("SELECT SUM(usage_count) as total FROM command_stats")
  ...
  async getUserCount(): Promise<number> {
    const result = await this.client.database.get("SELECT COUNT(*) as count FROM users")
    return result.count
  }
  ...
  async getEconomyStats(): Promise<{
    ...
    const totalCoins = await this.client.database.get("SELECT SUM(balance) + SUM(safe_balance) as total FROM users")
    ...
    const averageBalance = await this.client.database.get("SELECT AVG(balance + safe_balance) as average FROM users")
