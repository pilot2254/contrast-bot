...
export class Logger {
  ...
  constructor() {
    this.currentLevel = this.logLevels[config.logging.level as keyof typeof this.logLevels] || 1
  ...
  transaction(userId: string, type: string, amount: number, description: string): void {
    ...
    this.writeToFile("transaction", message, { userId, type, amount, description })
  }
