...
export class Database {
  ...
  async createUser(userId: string): Promise<void> {
    try {
      await this.run("INSERT OR IGNORE INTO users (user_id) VALUES (?)", [
        userId,
      ]);
    } catch (error) {
  ...
  async getUser(userId: string): Promise<any> {
    try {
      ...
      const user = await this.get("SELECT * FROM users WHERE user_id = ?", [
        userId,
      ]);
      if (!user) {
  ...
  async logTransaction(
    ...
    try {
      ...
      await this.run(
        "INSERT INTO transactions (user_id, type, amount, description, metadata) VALUES (?, ?, ?, ?, ?)",
        [userId, type, amount, description, metadataJson],
      );
    } catch (error) {
  ...
  async incrementCommandUsage(commandName: string): Promise<void> {
    try {
      ...
      throw new Error(
        `Failed to increment command usage for ${commandName}: ${error}`,
      );
    }
  }
  ...
  async cleanupExpiredData(): Promise<void> {
    try {
      ...
      await this.run("DELETE FROM rate_limits WHERE window_start < ?", [
        now - 60 * 60 * 1000,
      ]);
      ...
      await this.run("DELETE FROM error_logs WHERE timestamp < ?", [
        thirtyDaysAgo,
      ]);
