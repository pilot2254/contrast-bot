...
const command: Command = {
  data: new SlashCommandBuilder().setName("top-commands").setDescription("View the most used commands"),
  category: "misc",
  cooldown: 5,
  async execute(interaction: ChatInputCommandInteraction, client: ExtendedClient) {
    ...
    try {
      ...
      if (topCommands.length === 0) {
        const embed = CustomEmbedBuilder.info().setDescription("No command usage data available yet.")
