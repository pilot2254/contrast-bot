...
const command: Command = {
  ...
  async execute(
    ...
    try {
      ...
      const result = await gamblingService.playRussianRoulette(
        interaction.user.id,
      );
      ...
      if (result.survived) {
        const embed = CustomEmbedBuilder.success()
          ...
          .setDescription(
            "*Click*... The chamber was empty! You survived and won big!",
          )
          .addFields({
            name: "💰 Winnings",
        ...
        const embed = CustomEmbedBuilder.error()
          ...
          .setDescription(
            "*BANG!* The gun fired... You lost everything in your wallet!",
          )
          .addFields({
            name: "💸 Lost",
          ...
          .setFooter({
            text: "Better luck next time... Your safe balance is untouched.",
          });
