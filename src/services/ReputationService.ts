...
export class ReputationService {
  ...
  async getUserReputation(userId: string): Promise<ReputationCounts> {
    ...
    const reputation = await this.client.database.get<{ given: number; received: number }>(
      "SELECT given, received FROM reputation WHERE user_id = ?",
      [userId],
    )
  ...
  async takeReputation(takerId: string, targetId: string): Promise<void> {
    ...
    if (lastAction) {
      throw new Error("You can only affect this user's reputation (give/take) once every 24 hours")
    }
    ...
    await this.client.database.run("UPDATE reputation SET given = given + 1 WHERE user_id = ?", [takerId]) // Or a different counter for "actions_taken"
  ...
  async getReputationLeaderboard(type: "given" | "received", limit = 10): Promise<ReputationLeaderboardEntry[]> {
