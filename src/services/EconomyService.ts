...
export class EconomyService {
  ...
  async getBalance(userId: string): Promise<{ wallet: number; safe: number; safeCapacity: number }> {
  ...
  async addBalance(userId: string, amount: number, reason: string): Promise<number> {
    ...
    try {
      ...
      this.client.logger.error(`Error adding balance for user ${userId}:`, error)
      throw error
    }
  }
  ...
  async removeBalance(userId: string, amount: number, reason: string): Promise<number> {
    if (amount <= 0) throw new Error("Amount must be positive")
    ...
    try {
      ...
      this.client.logger.error(`Error removing balance for user ${userId}:`, error)
      throw error
    }
  }

  // Internal method that doesn't start a transaction
  private async _removeBalanceInternal(userId: string, amount: number, reason: string): Promise<number> {
    // Ensure user exists
    await this.client.database.createUser(userId)
  ...
  async transferBalance(
    ...
    if (amount <= 0) throw new Error("Amount must be positive")
    if (senderId === receiverId) throw new Error("You cannot transfer to yourself")

    // Check transaction limits
    if (
      config.economy.currency.maxTransactionAmount !== Number.POSITIVE_INFINITY &&
    ...
    try {
      return await this.client.database.transaction(async () => {
        ...
        // Remove from sender
        const senderBalance = await this._removeBalanceInternal(senderId, amount, `Transfer to ${receiverId}`)

        // Add to receiver
        const receiverBalance = await this._addBalanceInternal(receiverId, amount, `Transfer from ${senderId}`)

        // Add XP for transfer
        await this._addXPInternal(senderId, config.leveling.xpSources.transfer, "Transfer")
      ...
      if (
        ...
        (error.message.includes("Insufficient balance") || error.message.includes("Cannot transfer"))
      ) {
        throw error
      }
      this.client.logger.error(`Error transferring balance from ${senderId} to ${receiverId}:`, error)
      throw new Error("An error occurred while transferring funds")
    }
  }

  // Safe methods
  async depositToSafe(userId: string, amount: number): Promise<{ wallet: number; safe: number }> {
    if (amount <= 0) throw new Error("Amount must be positive")
    ...
    try {
      return await this.client.database.transaction(async () => {
        ...
        // Log transaction
        await this.client.database.logTransaction(userId, "deposit", amount, "Deposit to safe")
      ...
      if (
        error instanceof Error &&
        (error.message.includes("Insufficient balance") || error.message.includes("Your safe can only hold"))
      ) {
        throw error
      }
      this.client.logger.error(`Error depositing to safe for user ${userId}:`, error)
      throw new Error("An error occurred while depositing to your safe")
    }
  }

  async withdrawFromSafe(userId: string, amount: number): Promise<{ wallet: number; safe: number }> {
    if (amount <= 0) throw new Error("Amount must be positive")
    ...
    try {
      return await this.client.database.transaction(async () => {
        ...
        // Log transaction
        await this.client.database.logTransaction(userId, "withdraw", amount, "Withdraw from safe")
      ...
      if (
        error instanceof Error &&
        (error.message.includes("Insufficient safe balance") || error.message.includes("Your wallet cannot hold"))
      ) {
        throw error
      }
      this.client.logger.error(`Error withdrawing from safe for user ${userId}:`, error)
      throw new Error("An error occurred while withdrawing from your safe")
    }
  }

  async upgradeSafe(userId: string): Promise<{ tier: number; capacity: number; cost: number }> {
    try {
      return await this.client.database.transaction(async () => {
        // Ensure user exists
        await this.client.database.createUser(userId)
        ...
        const newCapacity =
          config.economy.safe.baseCapacity + (nextTier - 1) * config.economy.safe.capacityIncreasePerTier
      ...
      this.client.logger.error(`Error upgrading safe for user ${userId}:`, error)
  ...
  private async _addXPInternal(
    ...
    if (newXP >= requiredXP && currentLevel < config.leveling.maxLevel) {
      ...
      await this._addBalanceInternal(userId, config.leveling.levelUpBonus, `Level up bonus (Level ${newLevel})`)
    }
    ...
    try {
      return await this.client.database.transaction(async () => {
        ...
        if (claim) {
          ...
          await this._addXPInternal(userId, config.economy.daily.xpReward, "Daily reward")
          ...
          // Add balance and XP using internal methods
          await this._addBalanceInternal(userId, config.economy.daily.amount, "Daily reward")
          await this._addXPInternal(userId, config.economy.daily.xpReward, "Daily reward")
      ...
      if (error instanceof Error && error.message.includes("You can claim your daily reward in")) {
        throw error
      }
      this.client.logger.error(`Error claiming daily reward for user ${userId}:`, error)
  ...
  async getDailyStatus(userId: string): Promise<{ claimed: boolean; timeLeft: number; streak: number }> {
    try {
      ...
      this.client.logger.error(`Error getting daily status for user ${userId}:`, error)
      throw new Error("An error occurred while checking your daily reward status")
    }
  }
      ...
      return await this.client.database.transaction(async () => {
        ...
        if (claim) {
          ...
          if (timeSinceClaim < 7 * 24 * 60 * 60 * 1000) {
            ...
            throw new Error(`You can claim your weekly reward in ${days}d ${hours}h`)
          }
          ...
          // First time claiming
          await this.client.database.run("INSERT INTO claims (user_id, claim_type, last_claimed) VALUES (?, ?, ?)", [
            userId,
            "weekly",
            now,
          ])
        ...
        // Add balance and XP using internal methods
        await this._addBalanceInternal(userId, config.economy.weekly.amount, "Weekly reward")
        await this._addXPInternal(userId, config.economy.weekly.xpReward, "Weekly reward")

        return { amount: config.economy.weekly.amount }
      })
    } catch (error) {
      if (error instanceof Error && error.message.includes("You can claim your weekly reward in")) {
        throw error
      }
      this.client.logger.error(`Error claiming weekly reward for user ${userId}:`, error)
      throw new Error("An error occurred while claiming your weekly reward")
    }
  }

  async getWeeklyStatus(userId: string): Promise<{ claimed: boolean; timeLeft: number }> {
    try {
      // Ensure user exists
      await this.client.database.createUser(userId)

      const claim = await this.client.database.get("SELECT * FROM claims WHERE user_id = ? AND claim_type = 'weekly'", [
        userId,
      ])
      ...
      this.client.logger.error(`Error getting weekly status for user ${userId}:`, error)
      throw new Error("An error occurred while checking your weekly reward status")
    }
  }
      ...
      return await this.client.database.transaction(async () => {
        ...
        if (claim) {
          ...
          if (timeSinceClaim < 30 * 24 * 60 * 60 * 1000) {
            ...
            const days = Math.floor(timeLeft / (1000 * 60 * 60 * 24))
            throw new Error(`You can claim your monthly reward in ${days} days`)
          }
        ...
        // Add balance and XP using internal methods
        await this._addBalanceInternal(userId, config.economy.monthly.amount, "Monthly reward")
        await this._addXPInternal(userId, config.economy.monthly.xpReward, "Monthly reward")

        return { amount: config.economy.monthly.amount }
      })
    } catch (error) {
      if (error instanceof Error && error.message.includes("You can claim your monthly reward in")) {
        throw error
      }
      this.client.logger.error(`Error claiming monthly reward for user ${userId}:`, error)
      throw new Error("An error occurred while claiming your monthly reward")
    }
  }

  async getMonthlyStatus(userId: string): Promise<{ claimed: boolean; timeLeft: number }> {
    try {
      // Ensure user exists
      await this.client.database.createUser(userId)
      ...
      this.client.logger.error(`Error getting monthly status for user ${userId}:`, error)
      throw new Error("An error occurred while checking your monthly reward status")
    }
  }
  ...
  async claimYearly(userId: string): Promise<{ amount: number }> {
    try {
      return await this.client.database.transaction(async () => {
        ...
        // Add balance and XP using internal methods
        await this._addBalanceInternal(userId, config.economy.yearly.amount, "Yearly reward")
        await this._addXPInternal(userId, config.economy.yearly.xpReward, "Yearly reward")

        return { amount: config.economy.yearly.amount }
      })
    } catch (error) {
      if (error instanceof Error && error.message.includes("You can claim your yearly reward in")) {
        throw error
      }
      this.client.logger.error(`Error claiming yearly reward for user ${userId}:`, error)
      throw new Error("An error occurred while claiming your yearly reward")
    }
  }

  async getYearlyStatus(userId: string): Promise<{ claimed: boolean; timeLeft: number }> {
    try {
      // Ensure user exists
      await this.client.database.createUser(userId)

      const claim = await this.client.database.get("SELECT * FROM claims WHERE user_id = ? AND claim_type = 'yearly'", [
        userId,
      ])
      ...
      this.client.logger.error(`Error getting yearly status for user ${userId}:`, error)
      throw new Error("An error occurred while checking your yearly reward status")
    }
  }
}
