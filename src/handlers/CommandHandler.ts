  ...
  async loadCommands(): Promise<void> {
    ...
    for (const folder of commandFolders) {
      ...
      const commandFiles = readdirSync(folderPath).filter((file) =>
        process.env.NODE_ENV === "production" ? file.endsWith(".js") : file.endsWith(".ts") || file.endsWith(".js"),
      )
    ...
    try {
      if (!process.env.CLIENT_ID) {
        this.client.logger.error("CLIENT_ID environment variable is not set. Cannot register commands.")
        return
      }
      this.client.logger.info(`Registering ${commandsToRegister.length} slash commands...`)
      ...
      if (process.env.GUILD_ID) {
        ...
        await this.rest.put(Routes.applicationCommands(process.env.CLIENT_ID), { body: commandsToRegister })
      }
